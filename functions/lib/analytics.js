...
exports.backfillDailyForecasts = exports.snapshotProjections = void 0;
 ...
 */
function trainHoltWinters(data, m = 7, alpha = 0.3, beta = 0.05, gamma = 0.2) {
    if (data.length < 2 * m)
        return null; // Need ≥ 2 full periods
    ...
    let level2 = 0;
    for (let i = m; i < 2 * m && i < data.length; i++)
        level2 += data[i];
    level2 /= m;
    ...
    for (let p = 0; p < nCompletePeriodsForInit; p++) {
        let periodAvg = 0;
        for (let i = p * m; i < (p + 1) * m && i < data.length; i++)
            periodAvg += data[i];
        periodAvg /= m;
        if (periodAvg <= 0)
            continue;
        for (let i = 0; i < m; i++) {
            const idx = p * m + i;
            if (idx < data.length)
                seasonal[i] += (data[idx] / periodAvg);
        }
    }
    for (let i = 0; i < m; i++)
        seasonal[i] /= nCompletePeriodsForInit;
    ...
    for (let t = m; t < data.length; t++) {
        const y = data[t];
        if (y <= 0)
            continue; // Skip zero-revenue days (holidays, outages)
        const sIdx = t % m;
        const prevL = L;
 ...
 */
function hwForecast(state, h, seasonalStartIdx, m = 7) {
    const out = [];
...
//
function buildDowIndex(dailyDates, dailySales) {
...
//
function computeBiasCorrection(accuracyRatios, window = 14) {
    if (accuracyRatios.length === 0)
        return 1;
...
function computeDailyForecasts(opts) {
    ...
    const { cyMtdSales, cyMtdOrders, cyMtdUnits, lyDates, lyRevenue, lyMonthRevenue, lyMonthOrders, lyMonthUnits, todayIdx, fractional, totalDays, todayDow, biasRatios, } = opts;
    ...
    for (let i = 0; i < todayIdx; i++)
        lySalesMTD += ((_a = lyMonthRevenue[i]) !== null && _a !== void 0 ? _a : 0);
    lySalesMTD += ((_b = lyMonthRevenue[todayIdx]) !== null && _b !== void 0 ? _b : 0) * fractional;
    ...
    if (hwState && lyDates.length >= 14) {
        ...
        for (let i = todayIdx; i < totalDays; i++) {
            ...
            const hwBase = (_c = hwValues[hIdx]) !== null && _c !== void 0 ? _c : 0;
            // DoW adjustment
            const dayDow = (todayDow + (i - todayIdx)) % 7;
            const dowAdj = (_d = dowIndex[dayDow]) !== null && _d !== void 0 ? _d : 1;
            // Blend: 60% HW, 40% LY-raw (keeps seasonal spikes from LY)
            const lyRaw = (_e = lyMonthRevenue[i]) !== null && _e !== void 0 ? _e : hwBase;
            ...
            const lyDayOrd = (_f = lyMonthOrders[i]) !== null && _f !== void 0 ? _f : 0;
            const lyDayUnt = (_g = lyMonthUnits[i]) !== null && _g !== void 0 ? _g : 0;
            const lyDayRev = (_h = lyMonthRevenue[i]) !== null && _h !== void 0 ? _h : 0;
        ...
        for (let i = todayIdx + 1; i < totalDays; i++)
            lyRemaining += ((_j = lyMonthRevenue[i]) !== null && _j !== void 0 ? _j : 0);
        const budgetRemaining = lyRemaining * velocityMultiplier;
        for (let i = todayIdx + 1; i < totalDays; i++) {
            const lyDay = (_k = lyMonthRevenue[i]) !== null && _k !== void 0 ? _k : 0;
            const dayDow = (todayDow + (i - todayIdx)) % 7;
            const dowAdj = (_l = dowIndex[dayDow]) !== null && _l !== void 0 ? _l : 1;
            ...
            const lyDayOrd = (_m = lyMonthOrders[i]) !== null && _m !== void 0 ? _m : 0;
            const lyDayRev = (_o = lyMonthRevenue[i]) !== null && _o !== void 0 ? _o : 0;
            if (lyDayOrd > 0 && lyDayRev > 0) {
                dailyOrders[i] = Math.round(dailyRevenue[i] / (lyDayRev / lyDayOrd));
                dailyUnits[i] = Math.round(dailyOrders[i] * (((_p = lyMonthUnits[i]) !== null && _p !== void 0 ? _p : 0) / lyDayOrd));
            }
            else {
                dailyOrders[i] = Math.round(dailyRevenue[i] / globalAvgTicket);
                dailyUnits[i] = Math.round(dailyOrders[i] * globalAvgUnitsPerOrder);
    ...
    for (let i = todayIdx + 1; i < totalDays; i++)
        forecastedRemainder += dailyRevenue[i];
    // Add today's projected total (actuals so far + forecast for rest of today)
    const lyTodayFull = (_q = lyMonthRevenue[todayIdx]) !== null && _q !== void 0 ? _q : 0;
...
async function loadLyExtendedSeries(lyYear, month) {
    ...
    const snap = await shared_1.db.collection('analytics_daily')
        .where('date', '>=', startStr)
        .where('date', '<=', endStr)
        .orderBy('date', 'asc')
        .get();
    ...
    const monthRevenue = new Array(totalDaysInMonth).fill(0);
    const monthOrders = new Array(totalDaysInMonth).fill(0);
    const monthUnits = new Array(totalDaysInMonth).fill(0);
    const lyMonthStr = `${lyYear}-${String(month).padStart(2, '0')}`;
    snap.forEach((doc) => {
        var _a, _b, _c, _d;
        const d = doc.data();
        dates.push(d.date);
        revenue.push((_a = d.totalRevenue) !== null && _a !== void 0 ? _a : 0);
        if (d.month === lyMonthStr) {
            const dayNum = parseInt(d.date.split('-')[2], 10);
            if (dayNum >= 1 && dayNum <= totalDaysInMonth) {
                monthRevenue[dayNum - 1] = (_b = d.totalRevenue) !== null && _b !== void 0 ? _b : 0;
                monthOrders[dayNum - 1] = (_c = d.totalOrders) !== null && _c !== void 0 ? _c : 0;
                monthUnits[dayNum - 1] = (_d = d.totalUnits) !== null && _d !== void 0 ? _d : 0;
            }
        }
    });
...
// ─── snapshotProjections ──────────────────────────────────────────────────────
exports.snapshotProjections = functions
    .pubsub.schedule('55 23 * * *')
    .timeZone('America/Mexico_City')
    .onRun(async () => {
    const fs = shared_1.db;
    const now = new Date();
...
//
// ─────────────────────────────────────────────────────────────────────────────
exports.backfillDailyForecasts = functions
    ...
    const fs = shared_1.db;
    const targetMonth = (data === null || data === void 0 ? void 0 : data.month) || '';
    const overwrite = !!(data === null || data === void 0 ? void 0 : data.overwrite);
    ...
    for (let dayNum = 1; dayNum <= totalDays; dayNum++) {
        ...
        if (!overwrite && d.forecastRevenue !== undefined) {
            ...
            cySalesCumul += (_b = d.totalRevenue) !== null && _b !== void 0 ? _b : 0;
            cyOrdersCumul += (_c = d.totalOrders) !== null && _c !== void 0 ? _c : 0;
            cyUnitsCumul += (_d = d.totalUnits) !== null && _d !== void 0 ? _d : 0;
            skipped++;
            continue;
        }
        ...
        cySalesCumul += (_e = d.totalRevenue) !== null && _e !== void 0 ? _e : 0;
        cyOrdersCumul += (_f = d.totalOrders) !== null && _f !== void 0 ? _f : 0;
        cyUnitsCumul += (_g = d.totalUnits) !== null && _g !== void 0 ? _g : 0;
    }
    await batch.commit();
