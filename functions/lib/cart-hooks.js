...
exports.onReferralOrderCompleted = exports.processReviewRequests = exports.onOrderCompleted = exports.processRecoveryQueue = exports.onCartAbandoned = void 0;
/**
 * cart-hooks.ts
...
exports.onCartAbandoned = functions.firestore
    ...
    const existing = await shared_1.db.collection('recovery_queue')
    ...
    const step1Ref = shared_1.db.collection('recovery_queue').doc();
    batch.set(step1Ref, {
    ...
    const step2Ref = shared_1.db.collection('recovery_queue').doc();
    batch.set(step2Ref, {
        sessionId, email, phone, name, items, cartValue,
        step: 2,
    ...
    await shared_1.db.collection('coupons').doc(couponCode).set({
        code: couponCode,
        type: 'percentage',
        value: 5,
        isActive: true,
...
exports.processRecoveryQueue = functions.pubsub
    ...
    const snap = await shared_1.db.collection('recovery_queue')
    ...
    for (const docSnap of snap.docs) {
        ...
        const outboxRef = shared_1.db.collection('notification_outbox').doc();
        batch.set(outboxRef, {
...
exports.onOrderCompleted = functions.firestore
    ...
    await shared_1.db.collection('review_requests').doc(context.params.orderId).set({
...
exports.processReviewRequests = functions.pubsub
    ...
    var _a;
    const now = admin.firestore.Timestamp.now();
    const snap = await shared_1.db.collection('review_requests')
        .where('status', '==', 'pending')
        .where('sendAt', '<=', now)
        .limit(100)
        .get();
...
exports.onReferralOrderCompleted = functions.firestore
    ...
    const existing = await shared_1.db.collection('referral_rewards')
    ...
    const couponRef = shared_1.db.collection('coupons').doc(couponCode);
    batch.set(couponRef, {
        code: couponCode,
        type: 'fixed',
        value: 100,
    ...
    const rewardRef = shared_1.db.collection('referral_rewards').doc();
    batch.set(rewardRef, {
        referrerId,
    ...
    const referrerOrders = await shared_1.db.collection('orders')
