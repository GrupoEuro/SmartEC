...
exports.scheduledMeliSkuStats = exports.computeMeliSkuStats = void 0;
...
// ── Core computation ─────────────────────────────────────────────────────────
async function computeAndSaveSkuStats(windowDays = 90) {
    var _a, _b, _c, _d;
    const now = new Date();
    ...
    snap.forEach(doc => {
        ...
        const createdAt = order.createdAt instanceof admin.firestore.Timestamp
            ...
            : typeof order.createdAt === 'string'
                ...
                : null;
        if (!createdAt || createdAt < cut90)
            return;
        ordersRead++;
        const items = order.items || [];
        // Distribute the order fee proportionally across items (or just use per-item if available)
        const totalQty = items.reduce((s, i) => s + (i.quantity || 1), 0);
        const orderFeeTotal = (_a = order.marketplaceFee) !== null && _a !== void 0 ? _a : 0;
        items.forEach((item) => {
            var _a, _b, _c;
            const sku = item.sku || item.productId || 'UNKNOWN';
            const qty = item.quantity || 1;
            const revenue = (_b = (_a = item.subtotal) !== null && _a !== void 0 ? _a : (item.price * qty)) !== null && _b !== void 0 ? _b : 0;
            const feeProportion = totalQty > 0 ? qty / totalQty : 1;
            const fee = orderFeeTotal * feeProportion;
            if (!skuMap.has(sku)) {
                skuMap.set(sku, {
                    title: item.productName || item.name || sku,
                    mlItemId: ((_c = item.productId) === null || _c === void 0 ? void 0 : _c.startsWith('MLM')) ? item.productId : null,
                    mlmuId: null,
                    orders90: [],
                });
            }
    ...
    for (const [sku, data] of skuMap) {
        ...
        const sum = (arr, cutoff) => arr.filter(o => inWindow(o.date, cutoff))
            .reduce((acc, o) => ({
            qty: acc.qty + o.qty,
        ...
        const trend = vel30 > 0 && vel7 > vel30 * 1.15 ? 'rising' :
            vel30 > 0 && vel7 < vel30 * 0.85 ? 'falling' :
                'stable';
        const dates = all90.map(o => o.date).sort((a, b) => b.getTime() - a.getTime());
        const lastDate = (_b = (_a = dates[0]) === null || _a === void 0 ? void 0 : _a.toISOString().split('T')[0]) !== null && _b !== void 0 ? _b : null;
        const firstDate = (_d = (_c = dates[dates.length - 1]) === null || _c === void 0 ? void 0 : _c.toISOString().split('T')[0]) !== null && _d !== void 0 ? _d : null;
        // Enrich with FBM data
        const fbmData = data.mlItemId ? fbmByMlItemId.get(data.mlItemId) : null;
        const mlmuId = (fbmData === null || fbmData === void 0 ? void 0 : fbmData.mlmuId) || null;
        const title = (fbmData === null || fbmData === void 0 ? void 0 : fbmData.title) || data.title;
        const stat = {
            sku,
 ...
 */
exports.computeMeliSkuStats = functions
 ...
 */
exports.scheduledMeliSkuStats = functions
    .runWith({ timeoutSeconds: 300, memory: '512MB' })
...
//# sourceMappingURL=meli-orders-sync.js.map