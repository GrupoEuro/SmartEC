...
exports.processPayment = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const rateLimitRef = shared_1.db.collection('_rate_limits').doc(`pay_${emailKey}`);
        const now = Date.now();
    ...
    if (orderId) {
        try {
            const orderSnap = await shared_1.db.collection('orders').doc(orderId).get();
            if (orderSnap.exists) {
                ...
                const productSnaps = await Promise.all(items.map((item) => shared_1.db.collection('products').doc(item.productId).get()));
...
exports.refundOrder = functions.https.onCall(async (data, context) => {
    ...
    const cfgSnap = await shared_1.db.collection('config').doc('integrations').get();
...
exports.mpAuthUrl = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        await shared_1.db.collection('config').doc('integrations').set({ mercadopago: { oauthState: state } }, { merge: true });
...
exports.mpDiag = functions.https.onCall(async (data, context) => {
    ...
    const configSnap = await shared_1.db.collection('config').doc('integrations').get();
    ...
    if (step === 'save_credentials') {
        ...
        await shared_1.db.collection('config').doc('integrations').set({ mercadopago: { accessToken: newAt, publicKey: newPk } }, { merge: true });
