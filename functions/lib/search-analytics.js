...
exports.querySearchAnalytics = exports.backfillSearchEventsToBigQuery = exports.onSearchEventCreated = void 0;
/**
 * search-analytics.ts
...
async function ensureSearchBQSchema() {
    const dataset = shared_1.bigquery.dataset(bq_analytics_1.BQ_DATASET, { location: bq_analytics_1.BQ_LOCATION });
    const [dsExists] = await dataset.exists();
    if (!dsExists) {
        await dataset.create({ location: bq_analytics_1.BQ_LOCATION });
        console.log(`[SearchBQ] Created dataset ${bq_analytics_1.BQ_DATASET}`);
    }
    ...
    if (!tblExists) {
        await table.create({
            ...
            location: bq_analytics_1.BQ_LOCATION,
        });
        console.log(`[SearchBQ] Created table ${bq_analytics_1.BQ_DATASET}.${BQ_SEARCH_TABLE}`);
    }
}
...
exports.onSearchEventCreated = functions
    ...
    try {
        ...
        await shared_1.bigquery.dataset(bq_analytics_1.BQ_DATASET).table(BQ_SEARCH_TABLE).insert([row], { skipInvalidRows: true });
...
exports.backfillSearchEventsToBigQuery = functions
    ...
    await ensureSearchBQSchema();
    const table = shared_1.bigquery.dataset(bq_analytics_1.BQ_DATASET).table(BQ_SEARCH_TABLE);
    const BATCH_SIZE = 500;
    let inserted = 0;
    let skipped = 0;
    let errors = 0;
    // Paginate through all search_events docs
    let query = shared_1.db.collection('search_events').orderBy('timestamp').limit(BATCH_SIZE);
    let lastDoc = null;
    while (true) {
        const snap = lastDoc
...
exports.querySearchAnalytics = functions
    ...
    const DS = bq_analytics_1.BQ_DATASET;
