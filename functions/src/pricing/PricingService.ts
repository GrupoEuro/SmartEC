export class PricingService {
     ...
     */
    static calculateFloorPrice(baseCost: number, minimumMarginMultiplier: number = 1.2): number {
     ...
     */
    static calculateFinalListPrice(
        baseCost: number,
        floorMargin: number,
        shippingCost: number,
        fixedFees: number,
     ...
     */
    static distributeInboundFreight(
        freightCost: number,
        items: { id: string; weight: number; volume: number }[],
        strategy: 'volume' | 'weight'
    ): { id: string; allocatedCost: number }[] {
        if (freightCost < 0) throw new Error("Freight cost cannot be negative.");
