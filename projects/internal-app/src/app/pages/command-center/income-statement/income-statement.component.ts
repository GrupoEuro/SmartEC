...
export class IncomeStatementComponent implements OnInit {
    ...
    private getGrossMarginInsight(margin: number): string {
        if (margin >= 40) return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.GROSS_EXCELLENT';
        if (margin >= 30) return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.GROSS_GOOD';
        if (margin >= 20) return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.GROSS_FAIR';
        return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.GROSS_LOW';
    }

    private getNetMarginInsight(margin: number): string {
        if (margin >= 10) return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.NET_EXCELLENT';
        if (margin >= 5) return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.NET_GOOD';
        if (margin >= 2) return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.NET_FAIR';
        return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.NET_LOW';
    }

    private getEfficiencyInsight(efficiency: number): string {
        if (efficiency >= 75) return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.EFFICIENCY_EXCELLENT';
        if (efficiency >= 50) return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.EFFICIENCY_GOOD';
        if (efficiency >= 25) return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.EFFICIENCY_FAIR';
        return 'COMMAND_CENTER.INCOME_STATEMENT.HEALTH_SCORE.INSIGHTS.EFFICIENCY_LOW';
    }
